...
# When, PBC is applied, the average potential in the cell is zero, otherwise the boundaries are set to zero
...
#The difference between v3 is that I am attempting a non-mixed solution
...
parser.add_argument('--resolve', dest='resolve_eq', action='store_true', default=False)
...
parser.add_argument('--duallinear_solver', dest='duallin_solv', default='superlu_dist',choices=['superlu_dist', 'cg',"umfpack","mumps","petsc","gmres","minres","tfqmr","richardson","bicgstab"], help="Linear solver that will be used")
...
if bc_typ == 1 :
   ...
   if args.space == 0:
      ...
      Q = FunctionSpace(mesh, "CG", 2, constrained_domain=pb)
      R = FunctionSpace(mesh, 'R', 0, constrained_domain=pb)
   elif args.space == 2:
      Q = FunctionSpace(mesh, "CG", 3, constrained_domain=pb)
      R = FunctionSpace(mesh, 'R', 0, constrained_domain=pb)
   W = MixedFunctionSpace([Q, R])
elif bc_typ == 2 :
   if args.space == 0:
...
if args.chgmodel == 'gaussian':
    ...
    if bc_typ == 1 :
       ...
       rho = Expression(source_str, element=W.ufl_element())
    chg_int = assemble(rho*dx)
    print "Initial integral of the charge in mesh:", chg_int
    while 100.0*abs(chg_int -Chg_tot)/Chg_tot > 50 :
        print "Error in charge integral is too large"
        chg_center=dolfin.Point(Chg_locx,Chg_locy,Chg_locz)

        cell_markers = dolfin.CellFunction("bool", mesh)
        cell_markers.set_all(False)
        for cell in dolfin.cells(mesh):
            if cell.contains(chg_center) :
                cell_markers[cell] = True
        mesh = refine(mesh, cell_markers)
        chg_int = assemble(rho*dx(mesh))
        print "Integrated charge in refined mesh:", chg_int
    #now redefine the functions
    if bc_typ == 1 :
        if args.space == 0:
            Q = FunctionSpace(mesh, "CG", 1, constrained_domain=pb)
            R = FunctionSpace(mesh, 'R', 0, constrained_domain=pb)
...
if args.epsmodel == 'slab':
    ...
    if bc_typ == 1 :
       epsilon = Expression(eps_str, element=W.sub(0).ufl_element())
    elif bc_typ == 2 :
       epsilon = Expression(eps_str, element=W.ufl_element())
...
if mpiRank == 0 and Silence < 1:
  ...
  if args.aalgo == 'fenics' :
     print "Dual Linear solver: ",duallin_solv
  print "------------------------------------------------"
if mpiRank == 0 and Silence < 1:
...
if bc_typ == 1 :
   ...
   (v, nu) = TestFunctions(W)
   #problem
   a = (epsilon*inner(grad(u), grad(v))) * dx(mesh) - inner(u, nu) * dx(mesh) - inner(mu, v) * dx(mesh)
elif bc_typ ==2 :
   ...
   a = (epsilon*inner(grad(u), grad(v))) * dx(mesh)

L =  4 * np.pi * ((rho * v) * dx(mesh))
...
if args.aalgo == 'fenics' :
    ...
    #E integral
    if bc_typ == 1 :
       phi_new, mu_new = split(uw.leaf_node())
       phi_old, mu_old = split(uw.root_node())
       E_int_new = assemble(phi_new*rho*dx(mesh.leaf_node()))
       E_int_new = E_int_new * 0.5
       E_int_old = assemble(phi_old*rho*dx(mesh.root_node()))
       E_int_old = E_int_old * 0.5
    elif bc_typ == 2:
       E_int_new = assemble(u.leaf_node()*rho*dx(mesh.leaf_node()))
       E_int_new = E_int_new * 0.5
       E_int_old = assemble(u.root_node()*rho*dx(mesh.root_node()))
       E_int_old = E_int_old * 0.5
       if args.resolve_eq :
          (sigma,u2)=WS.split()
          E_resolve = assemble(u2*rhor*dx(resolv_mesh))
          E_resolve = E_resolve * 0.5
    chg_new_int = assemble(rho*dx(mesh.leaf_node()))
    chg_old_int = assemble(rho*dx(mesh.root_node()))
...
##
    ...
    if bc_typ == 1 :
       ...
       sigma, phi= uw.split()
    chg_ongrid = Function(W.sub(2))
    eps_ongrid = Function(W.sub(2))
...
if mpiRank == 0:
    if Silence < 1:
        if args.aalgo == 'fenics':
            ...
            if args.resolve_eq :
               print "Refined result:",E_resolve
            print "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
        elif args.aalgo == 'none':
            print "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="
            print "                 RESULTS                                          "
...
if args.aalgo == 'none':
       ...
       if args.dump_chg:
            ...
            filechg << chg_ongrid
       if args.dump_mesh:
            filemesh = File(fout_prefix + "-mesh.xml.gz")
            filemesh << mesh
       if args.dump_eps:
       ...
       if bc_typ == 1 :
          phi_old, mu_old = uw.root_node().split()
          phi_new, mu_new = uw.leaf_node().split()
       elif bc_typ == 2 :
          phi_old = u.root_node()
          phi_new = u.leaf_node()
       if args.dump_pot:
            ...
            filepot << phi_new
       if args.dump_chg:
            V_old = FunctionSpace(mesh.root_node(), "Lagrange", 1)
            chg_ongrid_old=Function(V_old)
            chg_ongrid_old.interpolate(rho)
            V_new = FunctionSpace(mesh.root_node(), "Lagrange", 1)
       ...
       if args.dump_mesh:
            filemesh_initial = File(fout_prefix + "-mesh-initial.xml.gz")
            filemesh_initial << mesh.root_node()
            filemesh_final = File(fout_prefix + "-mesh-final.xml.gz")
            filemesh_final << mesh.leaf_node()
       if args.dump_eps:
            V_old = FunctionSpace(mesh.root_node(), "Lagrange", 1)
